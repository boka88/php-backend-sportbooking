...
//******************************************************************************
	...
	$clann   = 0;	//sifra clana
	$igrac   = " ";	//ime i prezime
	$kojid   = 0;	//id broj upisa u rezervacije
	
	$upit = "SELECT * FROM rezervacije WHERE termin=". $bsatni . " and teren=" . $bteren . " and datum='" . $zadan."'";
	$podaci = mysqli_query($veza1,$upit);   
	
	$broj = mysqli_num_rows($podaci); 
	if ($broj>0){
		$teroj4	 = mysqli_fetch_array($podaci);
		$clann   = $teroj4['sifra_clana'];
		$igrac   = $teroj4['prezime_ime'];
		$kojid   = $teroj4['id'];
	}
	
	echo '[{"idbroj":'.$kojid.',"opera":' . $clann . ',"clapre":"' . $igrac . '"}]';
