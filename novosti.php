...
/* primer za unos probnih novosti u bazu "demo" mora biti taj dan kad se proba
*INSERT INTO novosti VALUES('Nova novost','2021-05-08','2021-05-08','U dva velika grada, Rijeci i Puli, Radnicka fronta na izbore izlazi s gradonacelnickim kandidatima, a u Zagrebu s listom za Gradsku skupstinu. Katarina Peovic, Goran Matic i Mate Kapovic za Novosti govore o programu RF-a u tim gradovima i ocekivanjima od izbora',
* 100.00,'Ostale novosti');
*
*
*
*
...
//***********************************************************	
	...
	$datumi = '';
	$porhh="";
	$upitx="SELECT * FROM novosti WHERE datumod<='".$danas."' and datumdo>='".$danas."' ORDER BY datumod";
	$resultatx = mysqli_query($veza1,$upitx);
	
	$rowcount=mysqli_num_rows($resultatx);
	if($rowcount>0){
		while ($xerow = mysqli_fetch_array($resultatx)){
		   ...
		   $datumi = substr($xerow['datumod'],8,2).'.'.substr($xerow['datumod'],5,2).'.'.substr($xerow['datumod'],0,4);
		   $datumi = $datumi . '-';
		   $datumi = $datumi.substr($xerow['datumdo'],8,2).'.'.substr($xerow['datumdo'],5,2).'.'.substr($xerow['datumdo'],0,4);
		   $jsonosnovni = $jsonosnovni.'"naziv":"'.$xerow['naziv'].'","datumi":"'.$datumi.'","opis":"'.$xerow['opis'].'",'.
						'"iznos":"'.$xerow['iznos'].'",'.'"ostalo":"'.$xerow['ostalo'].'"}';
			$ii = $ii + 1;
		}//end of while
	}else{
		$jsonosnovni = $jsonosnovni.'"naziv":"Nema novih novosti","opis":" ",'.
						'"ostalo":" "}';
	}
